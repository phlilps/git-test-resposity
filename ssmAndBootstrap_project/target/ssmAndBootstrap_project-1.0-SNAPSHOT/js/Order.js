        ...
        $.ajax({
            ...
            success: function (result) {//成功回调函数
                ...
                for (var i = 0; i < result[0].length; i++) {
                    ...
                    var   providers=ServerObject.providers==undefined?"null":ServerObject.providers
                    html += "<tr>" +
                        '<td><input name="ids" type="checkbox"></td>' +
                        "<td>" + ServerObject.id + "</td>" +
                        "<td>" + ServerObject.commodityCode + "</td>" +
                        "<td>" + ServerObject.commodityName + "</td>" +
                        "<td>" + ServerObject.commodityDesc + "</td>" +
                        "<td>" + ServerObject.commodityUnit + "</td>" +
                        "<td>" + ServerObject.commodityNum + "</td>" +
                        "<td>" + ServerObject.totalPrice + "</td>" +
                        "<td>" + ServerObject.isPayment + "</td>" +
                        "<td>" + providers.companyName+ "</td>" +
                        '<td class="text-center"> ' +
                        '<a href="javascript:void(0);" onclick="delUser(\'' +ServerObject.id + '\')" class="btn bg-olive btn-xs">删除</a> ' +
                        '<a href="../Order/return?id=' + ServerObject.id + '"  class="btn bg-olive btn-xs">修改</a> </td>'
                        + "</tr>"
